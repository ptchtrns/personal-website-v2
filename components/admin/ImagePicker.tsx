...
export function ImagePicker(
  ...
  return (
    <div class="@container">
      <div class="grid grid-cols-3 @sm:grid-cols-4 @md:grid-cols-6 gap-x-2 gap-y-3 max-h-72 overflow-y-auto p-2 border border-neutral-300 dark:border-neutral-600 rounded-lg">
        {allowNoImage && (
          <label class="relative flex aspect-square min-h-0 flex-col items-center justify-center gap-1 rounded-md border-2 border-transparent bg-neutral-100 text-neutral-400 cursor-pointer overflow-hidden has-[:checked]:border-primary dark:bg-neutral-800">
            ...
            <div class="absolute top-1.5 right-1.5">
              <Radio
                ...
                class="bg-white/90 dark:bg-neutral-900/90"
              />
            </div>
          </label>
        )}
        {images.map((image) => (
          ...
          >
            <img
              src={image.src}
              alt={image.alt ?? ""}
              class="absolute inset-0 w-full h-full object-cover bg-neutral-100 dark:bg-neutral-800"
            />
            <div class="absolute top-1.5 right-1.5">
              <Radio
                name={name}
